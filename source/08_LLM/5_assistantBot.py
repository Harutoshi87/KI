# assistant chatbot
# 1. assistant 생성(id 확보)
# 2. thread 객체 생성(id 확보) - 새 대화 시작하고 지금부터 대화 history 저장
# while True:
    # 3. thread에 사용자 질문 메시지 추가
    # 4. run 실행(assistant가 메시지를 읽고 응답 - 과금)
    # 5. 응답 메시지를 thread에서 추출하여 출력
# 6. 대화 이력을 history.txt로 파일 저장
import warnings
...
assistant_cs = client.beta.assistants.create(
    name='CustomerQnABot',
    instructions='당신은 고객 지원 챗봇입니다. 사용자 문의에 대해 간략히 답변해 주세요',
    model='gpt-4o-mini'
)
# 2. thread 생성 : 대화 시작(history 저장)
thread_cs = client.beta.threads.create()
print('어서오세요(exit을 입력하시면 종료됩니다)')
while True:
    ...
    # 6. 대화 이력을 data/ch7_history.txt로 파일 저장
