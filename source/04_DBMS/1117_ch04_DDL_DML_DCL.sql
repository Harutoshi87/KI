...
SELECT TO_CHAR(SALARY, '999,999,999.99') FROM MY_DATA;
...
-- DML 연습문제 page3
-- 1. EMP 테이블과 같은 구조와 같은 내용의 테이블 EMP01 을 생성 테이블이 있을시 제거한 후 모든 사원의 부서번호를 30 번으로 수정합니다
DROP TABLE EMP01;
...
-- 6. 부서명이 RESEARCH 인 사원을 모두 삭제하는 SQL 작성
...
-- (4) UNIQUE : 모든 행의 값이 유일해야 하나 NULL값은 중복 허용
-- (5) CHECK(조건) : 해당 조건이 만족(NULL값 허용)
-- (6) DEFAULT 기본값 : 기본값 설정
    -- (해당 열의 데이터를 입력하지 않고 INSERT하면 NULL이 들어갈 것을 DEFAULT값이 들어가도록 함)

-- DEPT1 & EMP1 테이블 생성
CREATE TABLE DEPT1( -- 제약 조건을 옆에 기술
    DEPTNO NUMBER(2)  PRIMARY KEY,
    DNAME VARCHAR(14) NOT NULL UNIQUE,
    LOC VARCHAR(13)   NOT NULL
);
CREATE TABLE EMP1( -- 제약 조건을 옆에 기술
    EMPNO NUMBER(4) PRIMARY KEY,
...
SELECT * FROM DEPT1;
SELECT * FROM EMP1;
-- 두 테이블 삭제 후 제약 조건을 아래에 기술
DROP TABLE EMP1; -- 자식 테이블부터 먼저 삭제
DROP TABLE DEPT1;
CREATE TABLE DEPT1( -- 제약 조건을 아래에 기술
    DEPTNO NUMBER(2),
...
CREATE TABLE EMP1( -- 제약 조건을 아래에 기술
    EMPNO NUMBER(4),
    ENAME VARCHAR(20) NOT NULL,
    JOB VARCHAR(20) NOT NULL,
    MGR NUMBER(4),
