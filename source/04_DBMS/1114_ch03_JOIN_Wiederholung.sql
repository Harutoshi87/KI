...
SELECT ENAME, SAL, GRADE, E.DEPTNO, DNAME, LOC
 ...
 SELECT ENAME, SAL, HIREDATE, GRADE
    FROM EMP, SALGRADE
    WHERE SAL BETWEEN LOSAL AND HISAL;
    -- EX4. 이름, 급여, 급여등급, 연봉, 부서명을 부서명순으로 정렬하여 출력.
...
SELECT ENAME, SAL, GRADE,((SAL+NVL(COMM, 0))*12) ANNUALSAL, DNAME
    ...
    -- EX5. 이름, 업무, 급여, 등급, 부서코드, 부서명 출력. 급여가 1000~3000사이.
...
SELECT ENAME, JOB, SAL, GRADE, E.DEPTNO, DNAME
    ...
    -- EX6. 이름, 급여, 등급, 입사일, 근무지. 81년에 입사한 사람. 등급 큰순
SELECT ENAME, SAL, GRADE, HIREDATE, LOC
    FROM EMP E, SALGRADE S, DEPT D
    WHERE D.DEPTNO=E.DEPTNO AND SAL BETWEEN LOSAL AND HISAL AND TO_CHAR(HIREDATE, 'RR')='81';
...
SELECT ENAME, JOB, DNAME, LOC
    FROM EMP E, DEPT D
    WHERE D.DEPTNO=E.DEPTNO AND ENAME LIKE '%L%';
...
SELECT W.ENAME||'의 상사는 ' || M.ENAME||'(이)다'
    FROM EMP W, EMP M
    WHERE W.MGR=M.EMPNO AND W.ENAME = 'SMITH';
  -- EX3. 매니저가 KING인 사원들의 이름과 직급을 출력하시오
...
SELECT W.ENAME, W.SAL, GRADE, DNAME, M.ENAME 직속상사명
    ...
    WHERE W.SAL BETWEEN LOSAL AND HISAL AND W.SAL >= 2000 AND W.MGR=M.EMPNO AND W.DEPTNO=D.DEPTNO;
...
SELECT W.ENAME, W.SAL, GRADE, DNAME, (W.SAL+NVL(W.COMM,0))*12 ANNUALSAL, M.ENAME 직속상사명
    FROM EMP W, EMP M, SALGRADE S, DEPT D
...
SELECT W.ENAME, M.ENAME 직속상사, MM.ENAME 상위상사
    FROM EMP W, EMP M, EMP MM
    WHERE W.MGR=M.EMPNO(+) AND M.MGR=MM.EMPNO(+);