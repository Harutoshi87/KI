...
--   commit을 하지 않는 한, DML은 얼마든지 취소가 가능하다.
--   rollback이란 키워드로 취소 가능
--   ex) select, insert, delete, update
-- 2.DDL(Data Definition Language) : 데이터 정의언어
--   DDL은 취소가 안된다.실행하면 바로 물리적 저장공간 바로 적용된다.
--   ex)create, drop, alter, truncate(테이블의 구조를 그대로 남긴 채 데이터를 다 지워준다.)
...
-- 아래 코드가 좀 생소하지만, 알아둘 필요가 있는 sub변수이다.
-- set구문 다음 @@붙으면 sub변수라고 생각을 하자.
